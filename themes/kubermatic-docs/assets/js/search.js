...
var searchModule = (function() {
  ...
  var initSearchProvider = function(searchField, searchResultsBox, clearButton) {
    ...
    var sectionTmpl = filterSection ? '<div class="search-results-item-section">in <em>{{`{{`}}productSection{{`}}`}}</em></div>' : "";

    var hits = instantsearch.widgets.hits({
      container: '.search-results',
      templates: {
        item: '<a href="{{`{{`}}relpermalink{{`}}`}}" title="{{`{{`}}title{{`}}`}}" class="search-results-item"><div class="search-results-item-title">{{`{{{`}}_highlightResult.title.value{{`}}}`}}</div><div class="search-results-item-content">{{`{{{`}}_snippetResult.content.value{{`}}}`}}</div>' + sectionTmpl + '</a>',
