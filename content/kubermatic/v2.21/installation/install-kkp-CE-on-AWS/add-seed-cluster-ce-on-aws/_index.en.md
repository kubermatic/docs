...
The Kubermatic Installer is the recommended way to setup new seed clusters. A manual installation is possible, however
more work needs to be done.

### Using the Installer

Similar to how the master cluster can be installed with the installer, run the `deploy` command. You still need to
...
try to talk to local token helper programs like `aws-iam-authenticator` for AWS.
...
When your cloud provider supports LoadBalancers, you can find the target hostname by looking at the
...
#nodeport-proxy   LoadBalancer   10.47.248.232   EXAMPLEEXAMPLEEXAMPLEEXAMPLE-EXAMPLE.eu-central-1.elb.amazonaws.com     80:32014/TCP,443:30772/TCP   449d
```
...
Create a CNAME record as needed pointing to the target:

```plain
*.kubermatic.kubermatic.example.com.   IN   CNAME   EXAMPLEEXAMPLEEXAMPLEEXAMPLE-EXAMPLE.eu-central-1.elb.amazonaws.com .
```
