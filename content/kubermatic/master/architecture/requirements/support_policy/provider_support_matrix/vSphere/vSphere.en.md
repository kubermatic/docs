...
When creating worker nodes for a user cluster, the user can specify an existing image. Defaults may be set in the [seed cluster `spec.datacenters.EXAMPLEDC.vsphere.endpoint`]({{< ref "../../../../kkp_clusters/seed_cluster" >}}).
...
During creation of a user cluster Kubermatic Kubernetes Platform (KKP) creates a dedicated VM folder in the root path on the Datastore (Defined in the [seed cluster `spec.datacenters.EXAMPLEDC.vsphere.datastore`]({{< ref "../../../../kkp_clusters/seed_cluster" >}})).
...
- At datacenter level (either with [Seed CRD]({{< ref "../../../../kkp_clusters/seed_cluster" >}})
    or [datacenters.yaml]({{< ref "../../../../kkp_clusters/seed_cluster" >}})) is possible to
