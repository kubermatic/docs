+++
title = "Add Seed Cluster for EE on Nutanix"
date =  2022-07-06T22:35:00+05:00
weight = 5
enterprise = true
+++
...
#kubermatic-fast      csi.nutanix.com       195d
#kubermatic-backup    csi.nutanix.com       195d
#ntnx-csi (default)   csi.nutanix.com       2y43d
```
...
a separate storage class with a different location/security level. For a cluster running on nutanix, an example class could
look as follows:
...
#seed-proxy-nutanix-seed-6dd5cc95cf-r6wvb               1/1     Running   0          80m
```
...
`hdu328tr.nutanix-seed.kubermatic.example.com` for the Seed from the previous step with `kubermatic.example.com` being the main domain
...
```plain
*.nutanix-seed.kubermatic.example.com.   IN   A   8.7.6.5
```