+++
title = "Benchmark on Kubernetes 1.35.2 with KKP 2.30.5"
date = 2026-07-01T07:33:18+00:00
+++

> CIS Benchmark **cis-1.12** applies to KKP **2.30.5** / Kubernetes **1.35.2**.
...
| Fail | 0 |
| Warn | 26 |
| ExpectedFail (matched by KKP exception) | 3 |
| PassWithConditions (matched by KKP exception) | 12 |
| Info | 0 |

Generated: 2026-07-01 · kube-bench v0.15.0
...
- **4.2.1** (Scanner Detection Gap) — KKP worker nodes configure kubelet via --config=/etc/kubernetes/kubelet.conf
  rather than command-line flags. kube-bench resolves $kubeletconf from a
  built-in path list that does not include /etc/kubernetes/kubelet.conf, so it
  falls back to /var/lib/kubelet/config.yaml (absent on KKP workers) and cannot
  read authentication.anonymous.enabled from the config file. The setting IS
  correctly set to false in the kubelet config; this is a scanner detection
  failure, not a real misconfiguration.
...
_KKP worker nodes configure kubelet via --config=/etc/kubernetes/kubelet.conf
rather than command-line flags. kube-bench resolves $kubeletconf from a
built-in path list that does not include /etc/kubernetes/kubelet.conf, so it
falls back to /var/lib/kubelet/config.yaml (absent on KKP workers) and cannot
read authentication.anonymous.enabled from the config file. The setting IS
