...
```bash
cosign verify quay.io/kubermatic/kubelb-manager:v1.5.0 \
...
```bash
cosign verify quay.io/kubermatic/helm-charts/kubelb-manager:v1.5.0 \
  --certificate-identity-regexp="^https://github.com/kubermatic/kubelb/.github/workflows/release.yml@refs/tags/v.*" \
  --certificate-oidc-issuer=https://token.actions.githubusercontent.com
```
...
curl -LO https://github.com/kubermatic/kubelb/releases/download/v1.5.0/checksums.txt
curl -LO https://github.com/kubermatic/kubelb/releases/download/v1.5.0/checksums.txt.sigstore.json
...
```bash
SBOM_DIGEST=$(oras discover --format json --artifact-type application/spdx+json \
  quay.io/kubermatic/kubelb-manager:v1.5.0 | jq -r '.referrers[0].digest')
...
```bash
cosign verify-attestation quay.io/kubermatic/kubelb-manager:v1.5.0 \
  --type spdxjson \
  --certificate-identity-regexp="^https://github.com/kubermatic/kubelb/.github/workflows/release.yml@refs/tags/v.*" \
  --certificate-oidc-issuer=https://token.actions.githubusercontent.com
```
...
curl -LO https://github.com/kubermatic/kubelb/releases/download/v1.5.0/kubelb_v1.5.0_linux_amd64.sbom.spdx.json
```
...
```bash
trivy image quay.io/kubermatic/kubelb-manager:v1.5.0
```
