...
* EvictionStrategy of `VMIs` is set to external, so VMI eviction needs to be handled properly by a custom
...
For KubeVirt clusters, we use Containerized Data Importer (CDI), which is a utility to import,
upload and clone Virtual Machine images for use with KubeVirt. At a high level, a persistent volume
...
The field that should be used to import those images is `sourceURL`.
